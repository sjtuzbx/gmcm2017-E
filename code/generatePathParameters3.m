function [f, intcon, A, b, Aeq, beq, lb, up] = generatePathParameters3(numOfEdges, numOfF, numOfPoints,edge, length, velocity, in, out, node)
    %对于问题三需要额外的约束
    ...
    f = zeros(18*numOfEdges+2*numOfF+size(idC,2), 1); %Add 6 more variables
    ...
    for i=Z(1):Z(6)
         for j=1:in{i}(1)
             A(counter2, 6*numOfEdges + in{i}(1+j)*3-k+1) = -1;
         end
    end
    b(counter2, 1) = 3-t(k) * 2 ;
    counter2 = counter2 + 1;
